...
RUN if [[ -n "${aptcacher}" ]]; then echo "Acquire::http::Proxy \"http://${aptcacher}:3142\";" >/etc/apt/apt.conf.d/01proxy && \
    ...
    apt-get update && apt-get install --no-install-recommends -qqy supervisor wget curl jq ca-certificates tzdata \
    dante-server net-tools \
    ...
    wget -nv -O /tmp/nordrepo.deb https://repo.nordvpn.com/deb/nordvpn/debian/pool/main/nordvpn-release_1.0.0_all.deb && \
    ...
    find /etc/apt/ -iname "*.list" -exec cat {} \; && \
    mkdir /run/nordvpn && \
